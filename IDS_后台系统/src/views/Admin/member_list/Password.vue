  ...
  >
    <div class="item_box" style="margin-bottom: .2rem;">
      <div class="item_list_box" style=" padding-top: .1rem; padding-bottom: 0px; max-width: 6rem;">
        <!-- 密码 -->
        <el-form-item :label="$t('new_members.pass')">
          <el-input type="password" :placeholder="$t('new_members.pass')" v-model="ruleForm.password"></el-input>
        </el-form-item>
        <!-- 提交 -->
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      vm.$refs[formName].validate(valid => {
        if (valid) {
          if (!!!vm.ruleForm.password) {
            vm.fnOpenMessageBox(vm.$t("new_members.regErr.realpass"), "error");
            return false;
          }
          let params = {
          ...
          vm.$main.loading = true;
          vm.$api.IDS_ADMIN_UPDATEUSERPASSWORD(params).then(res => {
            vm.$main.loading = false;
            if (res.code == 0) {
              // 修改成功
              vm.fnOpenMessageBox(
